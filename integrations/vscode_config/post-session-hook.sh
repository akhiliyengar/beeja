#!/usr/bin/env bash
# builder post-session-hook — append a signal record after every chat session
...
#   $AGENTS_SHADOW_ROOT/_signals/builder.jsonl

set -euo pipefail

SHADOW_ROOT="${AGENTS_SHADOW_ROOT:-$HOME/agents/shadow}"
...
if ! printf '%s' "$PAYLOAD" | grep -q '"build_artifact"\|builder --revise\|builder "'; then
  exit 0
fi

TS="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

# Use python (always present in builder envs) for safe JSON projection.
python - <<PY
